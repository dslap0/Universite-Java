 ...
 * descending order. The chain can be given from beginning to the end or the
 * opposite.
 *
...
public class ListeDoublementChainee
        implements IListeDoublementChainee, Serializable {
    /**
    ...
    // The order in which the list will be sorted (ascending or descending).
    ...
    public void ajouterNoeud(Item<?> item) {
        ...
        while (current != null) {
            ...
            if (Math.signum(item.compareTo(current.item)) == sensDeTri) {
                ...
                // previous node has his "suivant" set to the new node.
                if (before != null)
    ...
    /**
     * This method is used to give the list from beginning to the end.
     * 
     * @return The list as a string, from beginning to end.
     */
    @Override
    public String imprimerListeDuDebut() {
        ...
        // We loop through the list to add each node in order.
        while (node.suivant != null) {
            node = node.suivant;
            impression += "->";
            impression += node.item.getValeur();
        }
        impression += "\n";
    ...
    /**
     * This method is used to give the list from the end to the beginning.
     * 
     * @return The list as a string, from end to beginning.
     */
    @Override
